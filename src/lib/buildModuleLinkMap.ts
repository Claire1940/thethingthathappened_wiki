...
function matchScore(queryText: string, article: ArticleWithType, extraKeywords?: string[]): number {
  ...
  // Exact phrase match in title (stripped of "The Thing That Happened")
  const strippedQuery = normalizedQuery.replace(/the thing that happened\s*/g, '').trim()
  const strippedTitle = normalizedTitle.replace(/the thing that happened\s*/g, '').trim()
